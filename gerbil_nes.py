...
from gerbil import get_jsd_loss
import socket
import pickle
...
def calc_loss(trials,
    ...
    for _ in range(trials):
        ...
        trial_loss, trial_mmd_loss = get_jsd_loss(preds_E, y_E, preds_I, y_I, avg_step)
        loss_sum += trial_loss
...
def nes_multigaussian_optim(mean: torch.Tensor, cov: torch.Tensor, max_iter: int, samples_per_iter: int, y_E, y_I,
    ...
    with open(file_name, "w") as f:
        ...
        for i in tqdm(range(max_iter)):
            ...
            mean_loss, mean_MMD_loss = calc_loss(trials, weights_generator, network_executer, y_E, y_I, feed_forward=feed_forward)
            ...
            while len(samples) < samples_per_iter:  # Idealy, this could be done in parallel
                ...
                if accept_p < p:
                    ...
                    weights_valid = weights_generator.validate_weight_matrix()
                    if weights_valid == torch.tensor(0, device=device):
                        current_loss, _ = calc_loss(trials, weights_generator, network_executer, y_E, y_I, feed_forward=feed_forward)

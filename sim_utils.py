# This file is sim_utils.py + sim_utils_2.py in the jax version.
import numpy as np
...
#         rate = np.exp(-xm**2 - np.log(_g_ricci(-xm) - np.exp(xp**2 - xm**2) * _g_ricci(-xp)))
    ...
    inds = (xp > 0) & (xm <= 0)
    rate[inds] = 1 / ( _f_ricci(xp[inds]) + np.exp(xm[inds]**2) * _g_ricci(-xm[inds]) )
    rate[xp <= 0] = np.exp(-xm[xp <= 0]**2 - np.log(_g_ricci(-xm[xp <= 0]) 
                         - np.exp(xp[xp <= 0]**2 - xm[xp <= 0]**2) * _g_ricci(-xp[xp <= 0])))

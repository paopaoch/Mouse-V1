...
if __name__ == "__main__":
    ...
    # Initialise a list of mean_list
    def generate_mean_lists(start_mean_list, end_mean_list, num_samples=20):
        # TODO: FINISH THIS FUNCTION
        return [mean_list]

    mean_lists = generate_mean_lists([])

    for i, mean_list in enumerate(mean_lists):
        desc = f"Vary J value iteration {i}"
        mean, cov = nes_multigaussian_optim(mean, cov, 100, 12, y_E, y_I, device=device, neuron_num=1000, desc=desc,   # TODO: Add in log_file_name_option
                                            trials=1, alpha=0.1, eta_delta=1, avg_step_weighting=0.1, min_iter=20, 
                                            stopping_criterion_step=0.00001, stopping_criterion_tolerance=2)
        
        # TODO: Add mean and cov to a global log file
